...
export const createDelivery = async (req, res) => {
  try {
    ...
    // For now, create a placeholder URL that points to the frontend invoice generator
    const baseUrl =
      process.env.FRONTEND_URL ||
      process.env.BACKEND_URL ||
      "http://localhost:3001";
    invoiceUrl = `${baseUrl}/invoice-download/${delivery.trackingCode}`;
    ...
    // Enhanced email sending (PDF generation now handled on frontend)
    if (value.checkEmail && delivery.receiver?.email) {
      try {
        console.log(
          `Sending delivery creation email to ${delivery.receiver.email}`
        );

        // Use the delivery created email function (no PDF attachment from backend)
        const emailResult = await sendDeliveryCreatedEmail({
          ...delivery.toObject(),
          invoiceUrl,
        });
...
export const updateStatusAndLocation = async (req, res) => {
  try {
    ...
    // Enhanced email notification using the new professional email service
    if (value.checkEmail && value.status) {
      try {
        // Use the enhanced status update email function
        await sendStatusUpdateEmail(doc, value.status);
        console.log(
          `Enhanced status update email sent for ${doc.trackingCode}: ${value.status}`
        );
      } catch (emailError) {
        console.error(
          "Enhanced status update email failed:",
          emailError.message
        );
      }
    }
...
export const simpleEmailTest = async (req, res) => {
  try {
    ...
    await sendEmail({
      to: testEmail,
      subject: "Simple Test Email - Aegis Express Logistics",
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <p>Your Aegis Express Logistics email system is working correctly.</p>
      ...
      `,
      text: `Email Test Successful! This is a simple test email sent at: ${new Date().toLocaleString()}. Your Aegis Express Logistics email system is working correctly.`,
    });
...
export const testDeliveryEmail = async (req, res) => {
  try {
    ...
    const fakeDelivery = {
      ...
      status: "Pending",
      sender: {
        name: "Aegis Express Test Sender",
        email: "boltdropa@gmail.com",
